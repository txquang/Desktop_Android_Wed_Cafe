...
public class CT_HoaDon 
{
        ...
        try 
	{
	    ...
	    rs=truyvan.executeQuery("SELECT SP.TenSP AS 'Tên Móm ',CT.SoLuong AS 'SL',SP.GTDK AS 'Giá'\n" +
                                        "	FROM DongCT CT \n" +
                                        "		JOIN SanPham SP\n" +
                                        "			ON SP.MaSP = CT.MaSP\n" +
                                        "	\n" +
	    ...
	    for (int i=1; i<=3;i++)
	       colum.add(metaData.getColumnName(i));
            colum.add("Thành Tiền");//them cot thành tien
	    dtm.setColumnIdentifiers(colum);
            
            DecimalFormat formatter = new DecimalFormat("#,###,###");
            int SoLuong = 0, DonGia = 0, ThanhTien = 0, TongTien  = 0;
	    while (rs.next())
	    {
	        ...
	        for (int i=1; i<=3;i++)//so hang
                {
                   ...
                   if( i==2 ) 
                       SoLuong = Integer.parseInt(rs.getString(i));//láy sl
                   if( i==3 )
                   {
                       row.add(formatter.format(Double.valueOf(rs.getString(i)))+"đ");
                       DonGia= (int)Double.parseDouble(rs.getString(i));//laydon gia
                   }
                }
                ThanhTien = SoLuong * DonGia;
                TongTien+= ThanhTien;
                row.add(formatter.format(Double.valueOf(ThanhTien))+"đ");
                dtm.addRow(row);         
	    }
            row = new Vector<String>();
            String html= "<html>"
                    +"<p style=\"color:red;\">"+formatter.format(Double.valueOf(TongTien))+"đ"+"</p>"
                        +"</html>";
            row.add("Tổng Tiền");
            row.add("");
            row.add("");
...
"                           ORDER BY SoCT DESC";
    ...
    public void TaoHD(String SoBan, String MaNV, String MaHD) 
    {
        LoadDB();
